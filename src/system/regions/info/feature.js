import { html } from "lit";

export const leftImageFeature = html`
  <div class="cover cover-nav">
    <div class="principle">
      <div class="sidebar sidebar-split gap-m">
        <div class="frame photo">
          <img
            src="https://generative-placeholders.glitch.me/image?width=600&height=300"
            alt=""
            srcset=""
          />
        </div>
        <div class="fixed">
          <div class="prose">
            <p class="overline">Overline Text</p>
            <h2 class="region-headline">landing page title</h2>
            <p>
              Voluptate ipsum commodo culpa et nisi occaecat esse ut. Non mollit
              sunt cillum exercitation anim dolor voluptate. Sunt in eu
              exercitation pariatur elit. Quis magna enim ut id irure est. Magna
              laborum non fugiat aute est quis duis incididunt. Ad sint eiusmod
              sint tempor esse. Nulla quis dolore minim magna do est consectetur
              eu anim aute.
            </p>
          </div>
          <div class="stack gap-s">
            <div class="inline align-start gap-xs">
              <svg class="icon">
                <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
              </svg>
              <p>
                <strong>Hello world</strong> In ex velit eiusmod est proident
                anim dolor culpa nulla laborum.
              </p>
            </div>
            <div class="inline align-start gap-xs">
              <svg class="icon">
                <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
              </svg>
              <p>
                <strong>Hello world</strong> In ex velit eiusmod est proident
                anim dolor culpa nulla laborum.
              </p>
            </div>
            <div class="inline align-start gap-xs">
              <svg class="icon">
                <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
              </svg>
              <p>
                <strong>Hello world</strong> In ex velit eiusmod est proident
                anim dolor culpa nulla laborum.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
`;

export const rightImageFeature = html`
  <div class="cover cover-nav">
    <div class="principle">
      <div class="sidebar sidebar-split gap-m">
        <div class="fixed">
          <div class="prose">
            <p class="overline">Overline Text</p>
            <h2 class="region-headline">landing page title</h2>
            <p>
              Voluptate ipsum commodo culpa et nisi occaecat esse ut. Non mollit
              sunt cillum exercitation anim dolor voluptate. Sunt in eu
              exercitation pariatur elit. Quis magna enim ut id irure est. Magna
              laborum non fugiat aute est quis duis incididunt. Ad sint eiusmod
              sint tempor esse. Nulla quis dolore minim magna do est consectetur
              eu anim aute.
            </p>
          </div>
          <div class="stack gap-s">
            <div class="inline align-start gap-xs">
              <svg class="icon">
                <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
              </svg>
              <p>
                <strong>Hello world</strong> In ex velit eiusmod est proident
                anim dolor culpa nulla laborum.
              </p>
            </div>
            <div class="inline align-start gap-xs">
              <svg class="icon">
                <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
              </svg>
              <p>
                <strong>Hello world</strong> In ex velit eiusmod est proident
                anim dolor culpa nulla laborum.
              </p>
            </div>
            <div class="inline align-start gap-xs">
              <svg class="icon">
                <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
              </svg>
              <p>
                <strong>Hello world</strong> In ex velit eiusmod est proident
                anim dolor culpa nulla laborum.
              </p>
            </div>
          </div>
        </div>
        <div class="frame photo">
          <img
            src="https://generative-placeholders.glitch.me/image?width=600&height=300"
            alt=""
            srcset=""
          />
        </div>
      </div>
    </div>
  </div>
`;

export const cardGridFeature = html`
  <div class="region stack gap-s">
    <div class="prose center center-intrinsic">
      <h2 class="region-headline">Frequently Asked Questions</h2>
      <p>
        Officia sit tempor et exercitation ex laboris aute cupidatat pariatur
        adipisicing tempor. Nisi laboris nostrud quis sunt dolor.
      </p>
    </div>
    <div class="grid grid-3">
      <div class="card prose theme-primary-container">
        <svg class="icon block icon-l">
          <use href="src/elements/text/icon/icons.svg#accessibility"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="card prose theme-secondary-container">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="card prose theme-tertiary-container">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#browser-chrome-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
    </div>
  </div>
`;

export const blurbGridFeature = html`
  <div class="region stack gap-s">
    <div class="prose center center-intrinsic">
      <h2 class="region-headline">Frequently Asked Questions</h2>
      <p>
        Officia sit tempor et exercitation ex laboris aute cupidatat pariatur
        adipisicing tempor. Nisi laboris nostrud quis sunt dolor.
      </p>
    </div>
    <div class="grid grid-3">
      <div class="prose">
        <svg class="icon block icon-l">
          <use href="src/elements/text/icon/icons.svg#accessibility"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#browser-chrome-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon block icon-l">
          <use href="src/elements/text/icon/icons.svg#accessibility"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#browser-chrome-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
    </div>
  </div>
`;

export const offsetGridFeature = html`
  <div class="sidebar sidebar-split-s gap-xl">
    <div class="prose fixed">
      <h2 class="region-headline">Frequently Asked Questions</h2>
    </div>
    <div class="grid grid-2 gap-s">
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
      <div class="prose">
        <svg class="icon icon-l block">
          <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
        </svg>
        <h4 class="title-headline">Question Title</h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat. Non magna dolore do anim fugiat excepteur
          cillum ut veniam voluptate ad. Duis dolore adipisicing officia ipsum
          mollit. Cupidatat ex anim Lorem.
        </p>
      </div>
    </div>
  </div>
`;

export const offsetListFeature = html`
  <div class="sidebar sidebar-split-s gap-xl">
    <div class="prose fixed">
      <h2 class="region-headline">Frequently Asked Questions</h2>
    </div>
    <div class="grid grid-2 gap-s">
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
      <div class="prose">
        <h4 class="title-headline inline gap-xs align-center">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#f-check"></use>
          </svg>
          Question Title
        </h4>
        <p>
          Nulla nisi est incididunt nisi deserunt laborum. Et adipisicing et
          mollit minim et consequat.
        </p>
      </div>
    </div>
  </div>
`;

export const listFeature = html`
  <div class="region stack gap-s">
    <div class="prose center center-intrinsic">
      <h2 class="region-headline">Frequently Asked Questions</h2>
      <p>
        Officia sit tempor et exercitation ex laboris aute cupidatat pariatur
        adipisicing tempor. Nisi laboris nostrud quis sunt dolor.
      </p>
    </div>
    <div class="grid grid-2">
      <div class="stack gap-s">
        <div class="inline align-start gap-xs">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
          </svg>
          <p>
            <strong>Nulla nisi est</strong> incididunt nisi deserunt laborum.
          </p>
        </div>
        <div class="inline align-start gap-xs">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
          </svg>
          <p>
            <strong>Nulla nisi est</strong> incididunt nisi deserunt laborum.
          </p>
        </div>
        <div class="inline align-start gap-xs">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
          </svg>
          <p>
            <strong>Nulla nisi est</strong> incididunt nisi deserunt laborum.
          </p>
        </div>
      </div>
      <div class="stack gap-s">
        <div class="inline align-start gap-xs">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
          </svg>
          <p>
            <strong>Nulla nisi est</strong> incididunt nisi deserunt laborum.
          </p>
        </div>
        <div class="inline align-start gap-xs">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
          </svg>
          <p>
            <strong>Nulla nisi est</strong> incididunt nisi deserunt laborum.
          </p>
        </div>
        <div class="inline align-start gap-xs">
          <svg class="icon">
            <use href="src/elements/text/icon/icons.svg#c-remove-2"></use>
          </svg>
          <p>
            <strong>Nulla nisi est</strong> incididunt nisi deserunt laborum.
          </p>
        </div>
      </div>
    </div>
  </div>
`;
